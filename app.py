...
Handles routing, authentication, and task management.
...
# Never use a randomly generated key at runtime — it invalidates all sessions
# on every restart and is a security risk in production.
_secret_key = os.environ.get('SECRET_KEY')
if not _secret_key:
    raise RuntimeError(
        "SECRET_KEY environment variable is not set. "
        "Copy .env.example to .env and set a strong SECRET_KEY value."
    )
...
def get_tasks():
    ...
    status_arg = request.args.get('status', '')   # 'completed' | 'pending' | ''
...
@login_required
def add_task():
    """Create a new task after validating for time overlaps."""
...
@login_required
def update_task():
    """Update an existing task (full edit or status toggle)."""
    data    = request.get_json()
    task_id = data.get('id')
    user_id = session['user_id']
